...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    scores = X[i].dot(W) # the scores for each class
    ...
    for j in xrange(num_classes): # iterate over all wrong classes
      if j == y[i]:
        ...
        continue
      # calculate loss for the i-th example
      margin = scores[j] - correct_class_score + 1 # note delta = 1
      if margin > 0: # only take loss greater than 0
        diff_count += 1
  ...
  loss += 0.5 * reg * np.sum(W * W) # lambda(regularization strength) = 0.5
...
# num_classes = W.shape[1]
# incorrect_count = np.sum(margins > 0, axis=1)
